 ...
 * 
 * This namespace provides a wide range of methods for image loading, transformation, processing,
 * face detection, and facial landmark extraction using multiple algorithms including Dlib, OpenCV LBF,
 * and HAAR cascades.
 * 
 * @note All methods are and thread-safe when used with separate image instances.
 */
namespace ImageUtils
{
    // ============================================================================
     ...
     * @endcode
     */
    cv::Mat loadImage(const std::string& filePath, int flags = cv::IMREAD_COLOR);
     ...
     */
    bool saveImageWithQuality(const cv::Mat& image, const std::string& filePath, int quality);
     ...
     */
    cv::Mat resizeImageByHeight(const cv::Mat &image, int newHeight);
     ...
     */
    cv::Mat extractFace(const cv::Mat &image, const cv::Rect &faceRect, bool addPadding = true, double paddingScale = 0.1);
     ...
     */
    std::vector<cv::Point2f> detectFacialLandmarks(const cv::Mat& faceImage);
     ...
     */
    std::vector<cv::Point2f> detectFacialLandmarksHAAR(const cv::Mat& faceImage);
     ...
     */
    std::vector<cv::Point2f> generateDefaultLandmarks(const cv::Mat& faceImage);
     ...
     */
    std::vector<cv::Rect> detectFaces(const cv::Mat& grayImage);
     ...
     */
    std::vector<cv::Point2f> detectFacialLandmarksYuNet(const cv::Mat& faceImage);
     ...
     */
    std::vector<cv::Point2f> generateLandmarksFromKeyPoints(const std::vector<cv::Point2f>& key_points, const cv::Size& image_size);
     ...
     */
    void generateEyeLandmarks(const cv::Point2f& eye_center, float radius, std::vector<cv::Point2f>& landmarks);
     ...
     */
    void generateMouthLandmarks(const cv::Point2f& right_mouth, const cv::Point2f& left_mouth, float width, std::vector<cv::Point2f>& landmarks, bool inner);
     ...
     */
    std::vector<cv::Point2f> getFaceLandmarks(const cv::Mat& faceImage);
     ...
     */
    std::vector<cv::Point2f> getDefaultFaceLandmarks(const cv::Mat& faceImage);
     ...
     */
    bool hasSufficientResolution(const cv::Mat& image, int minWidth, int minHeight);
} // ImageUtils